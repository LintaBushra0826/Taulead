import React, { useState } from "react";
import axios from "axios";
import { Wrapper3, ContactHead, FormWrapper } from "./index.styled";
import bgPic from "../../../../assets/images/contact1.png";
import { Button, Col, Form, Input, Row } from "antd";
import TextArea from "antd/es/input/TextArea";

export default function Features() {
  const [formData, setFormData] = useState({
    fName: "",
    lname: "",
    email: "",
    phone: "",
    suggbox: "",
  });
  const API_BASE_URL = "http://localhost:3005";

  const handleInputChange = (event) => {
    const { name, value } = event.target;
    setFormData({ ...formData, [name]: value });
  };

  const handleSuggboxChange = (event) => {
    const value = event.target.value;
    setFormData({ ...formData, suggbox: value });
  };

  const handleSubmit = async () => {
    try {
      const response = await axios.post(`${API_BASE_URL}/send-email`, formData);
      if (response.status === 200) {
        alert("Email Sent Successfully!!");
        setFormData((prevData) => ({
          ...prevData,
          fName: "",
          lname: "",
          email: "",
          phone: "",
          suggbox: "",
        }));
      } else {
        console.error("Failed to send email.");
      }
    } catch (error) {
      console.error("Error sending email:", error);
    }
  };

  return (
    <Wrapper3 id="contactus">
      {/* <ContactHead>Contact Us</ContactHead> */}

      <FormWrapper>
        <ContactHead>Contact Us</ContactHead>
        <Form
          name="basic"
          layout="vertical"
          autoComplete="off"
          title="Contact US"
        >
          <Row gutter={20}>
            <Col span={9}>
              <Form.Item
                labelCol={{ span: 24 }}
                label={<span style={{ color: "#D3CCE3" }}>First Name</span>}
                name="fName"
              >
                <Input
                  name="fName"
                  defaultValue={formData.fName}
                  onChange={handleInputChange}
                  style={{
                    color: "#D3CCE3",
                    fontSize: "24px",
                    backgroundColor: "transparent",
                    borderColor: "#D3CCE3",
                  }}
                />
              </Form.Item>
            </Col>

            <Col span={8}>
              <Form.Item
                labelCol={{ span: 24 }}
                label={<span style={{ color: "#D3CCE3" }}>Last Name</span>}
                name="lname"
                style={{ color: "D3CCE3" }}
              >
                <Input
                  name="lname"
                  value={formData.lname}
                  onChange={handleInputChange}
                  style={{
                    color: "#D3CCE3",
                    fontSize: "24px",
                    backgroundColor: "transparent",
                    borderColor: "#D3CCE3",
                  }}
                />
              </Form.Item>
            </Col>

            <Col span={9}>
              <Form.Item
                labelCol={{ span: 24 }}
                label={<span style={{ color: "#D3CCE3" }}>Email Address</span>}
                name="email"
              >
                <Input
                  name="email"
                  value={formData.email}
                  onChange={handleInputChange}
                  style={{
                    color: "#D3CCE3",
                    fontSize: "24px",
                    backgroundColor: "transparent",
                    borderColor: "#D3CCE3",
                  }}
                />
              </Form.Item>
            </Col>

            <Col span={8}>
              <Form.Item
                labelCol={{ span: 24 }}
                label={<span style={{ color: "#D3CCE3" }}>Phone</span>}
                name="phone"
              >
                <Input
                  name="phone"
                  value={formData.phone}
                  onChange={handleInputChange}
                  style={{
                    color: "#D3CCE3",
                    fontSize: "24px",
                    backgroundColor: "transparent",
                    borderColor: "#D3CCE3",
                  }}
                />
              </Form.Item>
            </Col>
          </Row>

          <Row gutter={20}>
            <Col span={17}>
              <Form.Item
                labelCol={{ span: 24 }}
                label={
                  <span style={{ color: "#D3CCE3" }}>Comments/Suggestions</span>
                }
                name="suggbox"
              >
                <TextArea
                  onChange={handleSuggboxChange}
                  className="transparent-textarea"
                  style={{
                    color: "#D3CCE3",
                    fontSize: "24px",
                    backgroundColor: "transparent",
                    borderColor: "#D3CCE3",
                  }}
                />
              </Form.Item>
            </Col>
          </Row>
        </Form>

        <Button className="contactbutton" onClick={handleSubmit}>
          Submit
        </Button>
      </FormWrapper>
      <img
        src={bgPic}
        alt="bgPic"
        className="bg-image"
        style={{
          position: "absolute",
          width: "30%",
          height: "60vh",
          marginRight: "-990px",
        }}
      />
    </Wrapper3>
  );
}
